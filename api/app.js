// npm init -y & npm install express mongoose cors body-parser jsonwebtoken dotenv
// npm init -y & npm install express mysql cors body-parser jsonwebtoken dotenv
// admin123
// P@ssw0rd
...
function user(req,res,next)
{
  ...
  {
    res.json({response:"Failed",data:"not allowed",responseMessage:"please login to continue"})
  }
}
...
app.post("/login",(req,res) => {
  ...
  con.query(sql,(err,data) =>{
    if(err){res.json({response:"Failed",responseMessage:"failed to login"})}
     else
     {
      if(data.length>0)
      {
        jwt.sign({id:data[0].id},secret,(e,token) =>{
          if(e){res.json({response:"Failed",responseMessage:"failed to login"})}
           else
           {
            res.json({response:"Success",responseMessage:"login success",user:{...data[0],token}})
           } 
        })
      }
      else
      {
        res.json({response:"Failed",responseMessage:"failed to login"})
      }
     } 
  })
})
app.post("/signup",(req,res) => {
  ...
  let sql2=`insert into users(firstname,lastname,email,phone,password)values('${firstname}','${lastname}','${email}','${phone}','${password}')`

  con.query(sql1,(e1,d1) =>{
    if(e1){res.json({response:"Failed",responseMessage:"failed to signup"})}
     else
     {
      if(d1.length>0)
      {
        res.json({response:"Failed",responseMessage:"signup failed email already exists"})
      }
      else
      {
          con.query(sql2,(e2,d2) =>{
            if(e2){res.json({response:"Failed",responseMessage:"signup failed"})}
             else
             {
              res.json({response:"Success",responseMessage:"signup successfull"})
             } 
          })
      }
     } 
...
app.post("/cart",user,(req,res) => {
  ...
  let sql1=`select * from cart where uid='${uid}' and pid='${pid}'`
  let sql2=`insert into cart(uid,pid,qty) values('${uid}','${pid}',1)`
  let sql3=`update cart set qty=qty+1 where uid='${uid}' and pid='${pid}'`
  let sql4=`select * from cart where uid='${uid}'`
  // starts
  con.query(sql1,(e1,d1) =>{
    if(e1){res.json({response:"Failed",responseMessage:"Product added to cart failed"})}
     else
     {
        if(d1)
        {
          ...
          { 
            con.query(sql2,(e2,d2) =>{
              if(e2){res.json({response:"Failed",responseMessage:"Product added to cart failed"})}
               else
               {
                con.query(sql4,(e3,d3) =>{
                  if(e3){res.json({response:"Failed",responseMessage:"Product added to cart failed"})}
                  else 
                    res.json({response:"Success",responseMessage:d3})
                })
               } 
            })
          }
        ...
        {
          res.json({response:"Failed",responseMessage:"Product added to cart failed"})
        }
     } 
  })
  // ends
...
app.get("/cart",user,(req,res) => {
  ...
  con.query(sql,(e,d) =>{
    if(e){res.json({response:"Failed",responseMessage:"signup failed"})}
     else
     {
      res.json({response:"Success",responseMessage:d})
     } 
  })
})
app.get("/qty/:id/:qty",user,(req,res) => {
  let uid=req.user_id
  let {id,qty}=req.params
  let sql=`update cart set qty='${qty}' where cid='${id}' and uid='${uid}'`
  let sql4=`select * from cart where uid='${uid}'`
  con.query(sql,(e,d) =>{
    if(e){res.json({response:"Failed",responseMessage:[]})}
     else
     {
      con.query(sql4,(e3,d3) =>{
        if(e3){res.json({response:"Failed",responseMessage:[]})}
        else 
          res.json({response:"Success",responseMessage:d3})
      })
     } 
...
app.get("/remove/:id",user,(req,res) => {
  let uid=req.user_id
  let {id}=req.params
  let sql=`delete from cart where cid='${id}'`
  let sql4=`select * from cart where uid='${uid}'`
  con.query(sql,(e,d) =>{
    if(e){res.json({response:"Failed",responseMessage:[]})}
     else
